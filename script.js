  ...
  async handleSearch() {
    ...
    try {
      ...
      if (universities.length === 0) {
        this.showError(
          `No universities found for "${country}". Please check the country name and try again.`
        );
      ...
      if (error.response?.status === 404) {
        this.showError(
          `No universities found for "${country}". Please check the country name.`
        );
      } else if (error.code === "NETWORK_ERROR" || !navigator.onLine) {
        this.showError("Please check your internet connection and try again.");
      } else {
        this.showError(
          "Unable to fetch universities. The service might be temporarily unavailable. Please try again later."
        );
      }
      this.filtersContainer.style.display = "none";
    } finally {
  ...
  async fetchUniversities(country) {
    try {
      ...
      response = await fetch(
        `${this.apiUrl}?name=${encodeURIComponent(country)}`
      );
  ...
  addSearchSuggestions() {
    ...
    this.countryInput.addEventListener("input", (e) => {
      ...
      const matchingSuggestions = suggestions.filter(
        (country) => country.toLowerCase().includes(value) && value.length > 0
      );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log(
    '💡 Tip: Try searching for countries like "India", "United States", or "Canada"'
  );
});
...
const performanceObserver = new PerformanceObserver((list) => {
  for (const entry of list.getEntries()) {
    if (entry.entryType === "navigation") {
      console.log(
        `⚡ Page loaded in ${entry.loadEventEnd - entry.loadEventStart}ms`
      );
    }
  }
});
